...
class InferenceLoop:
    ...
    def __init__(self, args: Namespace) -> "InferenceLoop":
        self.args = args.diffbir
        self.pisa_args = args.pisa
        self.gfpgan_args = args.gfpgan
        self.ffanet_args = args.ffanet
        self.ftn_args = args.ftn
        self.ftn = None
    ...
    def load_cldm(self) -> None:
        ...
        self.cldm: ControlLDM = instantiate_from_config(
            OmegaConf.load(os.path.join(current_dir, "../../configs/inference/cldm.yaml"))
        )

        # load pre-trained SD weight
        if self.args.version == "v2.1":
            sd_weight = load_model_from_url(MODELS["sd_v2.1_zsnr"], self.args.model_dir)
        else:
            # v1, v2
            sd_weight = load_model_from_url(MODELS["sd_v2.1"], self.args.model_dir)
        ...
        if self.args.version == "v1":
            if self.args.task == "face":
                control_weight = load_model_from_url(MODELS["v1_face"], self.args.model_dir)
            elif self.args.task == "sr" or self.args.task == "denoise":
                control_weight = load_model_from_url(MODELS["v1_general"], self.args.model_dir)
            else:
                raise ValueError(
            ...
            control_weight = load_model_from_url(MODELS["v2"], self.args.model_dir)
        else:
            # v2.1
            control_weight = load_model_from_url(MODELS["v2.1"], self.args.model_dir)
        ...
        if self.args.version in ["v1", "v2"]:
            config = os.path.join(current_dir,"../../configs/inference/diffusion.yaml")
        else:
            config = os.path.join(current_dir,"../../configs/inference/diffusion_v2.1.yaml")
