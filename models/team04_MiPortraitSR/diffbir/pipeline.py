...
from ..PiSA.src.my_utils.wavelet_color_fix import adain_color_fix
import pyiqa
...
from ..FTN.face_tone_net import FaceToneNet
...
def check_scores(lq_scores, thresholds):
    for score, threshold in zip(lq_scores, thresholds):
        if score >= threshold:
            return False
    return True

def sharpen_and_color_enhance(img_tensor: torch.Tensor, device, sharpness_radius = 0, 
                              sharpeness_percent=50, color_enhance=1.1) -> torch.Tensor:
    to_pil = ToPILImage()
    img_tensor = img_tensor.squeeze(0)
    img_tensor = (img_tensor * 255).clamp(0, 255).to(torch.uint8)
    pil_sample = to_pil(img_tensor)
    unsharp_mask = ImageFilter.UnsharpMask(radius=sharpness_radius, percent=sharpeness_percent, threshold=0)
    pil_sample = pil_sample.filter(unsharp_mask)
    enhancer = ImageEnhance.Color(pil_sample)
    pil_sample = enhancer.enhance(color_enhance)
    to_tensor = transforms.ToTensor()
    img_tensor = to_tensor(pil_sample)
...
class Pipeline:
    ...
    def run(
        ...
        with VRAMPeakMonitor("applying cleaner"):
            cond_img = self.apply_cleaner(lq_tensor, lq)
            #cond_img = lq_tensor
        ...
        #save_image(sample, '/home/lyy/Code/submit/mid/3.png') 
        # Do sharpen and color enhance before stage3
        if hasattr(self.enhance_params, "enhancer2"):
            sample = sharpen_and_color_enhance(sample, device=sample.device, 
                    sharpness_radius=self.enhance_params.enhancer2[0],
                    sharpeness_percent=self.enhance_params.enhancer2[1],
                    color_enhance=self.enhance_params.enhancer2[2])  
            #save_image(sample, '/home/lyy/Code/submit/mid/4.png') 
        if self.ftn is not None:
            #save_image(sample, '/home/lyy/Code/facerestoration/results/v2_test1/4.png')   

            normalize(sample, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), inplace=True)
            sample = self.ftn(sample)
            sample = sample * 0.5 + 0.5
   
            #save_image(sample, '/home/lyy/Code/facerestoration/results/v2_test1/5.png')
...
class MiFRPipeline(Pipeline):
    def __init__(
        ...
        self.ftn = ftn
        self.iqa_helper = []
        if self.cleaner.gfpgan_cfg.metrics is not None:
            self.iqa_helper = [pyiqa.create_metric(metric_name, device=device) for metric_name in self.cleaner.gfpgan_cfg.metrics]
        self.enhance_params = enhance_params

    def apply_cleaner(
        self, lq: torch.Tensor, lq_image
    ) -> torch.Tensor:
        
        lq_scores = []
        for metric in self.iqa_helper:
            score = metric(lq)
            lq_scores.append(score.item())
        
        # Do PiSA Restoration
        inference_time, output = self.cleaner.pisa(self.cleaner.pisa_cfg.default, lq * 2 - 1, prompt='')
        output = output * 0.5 + 0.5
        output = torch.clip(output, 0, 1)
        #save_image(output, '/home/lyy/Code/1/2/facerestoration/results/test111/0.png')

        to_pil = ToPILImage()
        output_pil = to_pil(output.squeeze(0).cpu())
        output_pil = adain_color_fix(target=output_pil, source=lq_image[0])
        output = TF.to_tensor(output_pil).unsqueeze(0).to(lq.device)
        
        #save_image(output, '/home/lyy/Code/1/2/facerestoration/results/test111/1.png')

        # Do FFANet dehaze
        normalize(output,(0.64, 0.6, 0.58), (0.14, 0.15, 0.152), inplace=True)
        output = self.cleaner.ffanet(output)
        output = torch.clamp(output, 0, 1)
        #save_image(output, '/home/lyy/Code/1/2/facerestoration/mid/2.png')

        # Do GFPGAN Restoration (if needed)
        if len(self.iqa_helper) == 0:   
            normalize(output, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), inplace=True)
            output = self.cleaner.gfpgan(output, return_rgb=False, weight=self.cleaner.gfpgan_cfg.weight)[0]
            output = output*0.5 + 0.5
            #save_image(output, '/home/lyy/Code/1/2/facerestoration/mid/3.png')
        else:
            if check_scores(lq_scores, self.cleaner.gfpgan_cfg.cleaner_thresholds):
                normalize(output, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), inplace=True)
                output = self.cleaner.gfpgan(output, return_rgb=False, weight=self.cleaner.gfpgan_cfg.weight)[0]
                output = output*0.5 + 0.5
            elif hasattr(self.enhance_params, "enhancer1"):
                output = sharpen_and_color_enhance(output, device=output.device, 
                                          sharpness_radius=self.enhance_params.enhancer1[0],
                                          sharpeness_percent=self.enhance_params.enhancer1[1],
                                          color_enhance=self.enhance_params.enhancer1[2])
        return output
