...
class Pari_Image_dataset(Dataset):
    ...
    def __init__(self, image_path, sr_factor, patch_size, batch_size, num_scale):
        ...
        for i in range(self.num_scale - 1):
            # resize_transform = A.Resize(height=h_list[1], width=w_list[i])
            self.image_list.append(resize(self.image, target_shape=(h_list[1], w_list[1]), interpolation=cv2.INTER_LINEAR))
    ...
    def __getitem__(self, idx):
        ...
        lr_sample = [resize(img, (self.patch_size // self.sr_factor, self.patch_size // self.sr_factor), cv2.INTER_LINEAR) for img in hr_sample]
        lrup_sample = [resize(img, (self.patch_size, self.patch_size), interpolation=cv2.INTER_CUBIC) for img in lr_sample]
        # 将图像对转化为tensor数据
...
class Single_Image_dataset(Dataset):
    ...
    def __init__(self, image_path, sr_factor):
        ...
        self.sr_factor = sr_factor
        # resize_transform = A.Resize(height=h*self.sr_factor, width=w*self.sr_factor, interpolation=cv2.INTER_CUBIC)
        self.image_upscale = resize(self.image, target_shape=(h*self.sr_factor,w*self.sr_factor), interpolation=cv2.INTER_CUBIC)
