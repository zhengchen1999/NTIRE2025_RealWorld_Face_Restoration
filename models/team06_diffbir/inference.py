...
# DEFAULT_POS_PROMPT = (
#     "sufficient detail, high-resolution, detailed face, natural skin texture, sharp focus, realistic lighting, clear facial features, defined jawline, natural expression, proportional face, beautiful face, well-lit, high clarity, high aesthetic quality"
#     "natural proportions, detailed eyes, realistic shadows, natural colors, high-quality details, normal facial proportions"
# )

# DEFAULT_NEG_PROMPT = (
#     "blurry, pixelated, low-resolution, distorted features, excessive noise, disproportionate facial features, plastic-like skin, cartoonish, unrealistic expressions, misshapen eyes or nose, blur hair, artifacts eyes, artifacts teeth"
#     "blurry, unnatural colors, exaggerated shadows, artifacts, overexposed areas, asymmetrical face, unnatural lighting, harsh shadows, warped facial proportions, noisy background, out-of-focus, unnatural textures, unrealistic proportions, strange or distorted facial features, blur hair, artifacts eyes, artifacts teeth"
# )
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--swindegrad_pt",
        type=str,
        help="DiffBIR degrad version pt.",
    )
    parser.add_argument(
        "--train_cfg",
        type=str,
    ...
    parser.add_argument("--batch_size", type=int, default=2, help="Nothing to say.")
    # guidance parameters
    parser.add_argument(
    ...
    parser.add_argument(
        "--device", type=str, default="cuda", choices=["cpu", "cuda", "mps","cuda:0", "cuda:1", "cuda:2", "cuda:3"]
    )
    parser.add_argument(
...
def run_inference_from_args(args: Namespace):
    """
    Function to run inference given parsed arguments (args).
    This allows modular usage of the inference logic without relying on the command line.
    """
    # Check and set device
    args.device = check_device(args.device)
    set_seed(args.seed)

    print(f"Running inference with arguments: {args}")

    # Select the appropriate inference loop based on the task
    if args.version != "custom":
        loops = {
            "sr": BSRInferenceLoop,
            "denoise": BIDInferenceLoop,
        ...
        inference_loop = loops[args.task](args)
    else:
        inference_loop = CustomInferenceLoop(args)
