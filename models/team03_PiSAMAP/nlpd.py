...
class NLPD(nn.Module):
    ...
    def __init__(self, channels=1, test_y_channel=True, k=6, filt=None, low_freq=False):
    ...
    def nlpd(self, x1, x2):
        ...
        y1 = self.pyramid(x1)
        y2 = self.pyramid(x2)
        if self.low_freq: #only compute low frequency components
            y1 = y1[-1:]
            y2 = y2[-1:]
        total = []
...
if __name__ == '__main__':
    ...
    model = NLPD(channels=3, test_y_channel=False, low_freq=True).to(device)
