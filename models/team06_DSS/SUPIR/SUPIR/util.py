...
def create_SUPIR_model(config_path, SUPIR_sign=None, load_default_setting=False, model_dir=''):
    config = OmegaConf.load(config_path)

    config.SDXL_CKPT = os.path.join(model_dir, 'supir', 'sd_xl_base/sd_xl_base_1.0_0.9vae.safetensors') ############################
    config.SUPIR_CKPT_Q = os.path.join(model_dir, 'supir', 'SUPIR-v0Q.ckpt')
    # 遍历 emb_models 并修改对应的路径
    for emb_model in config.model.params.conditioner_config.params.emb_models:
        if emb_model["target"] == "sgm.modules.encoders.modules.FrozenCLIPEmbedder":
            emb_model["params"]["sdxl_clip1_path"] = os.path.join(model_dir, 'supir', 'clip-vit-large-patch14')
        elif emb_model["target"] == "sgm.modules.encoders.modules.FrozenOpenCLIPEmbedder2":
            emb_model["params"]["sdxl_clip2_ckpt_pth"] = os.path.join(model_dir, 'supir', 'CLIP-VIT-bigG-14-laion2B-39B-b160k/open_clip_pytorch_model.bin')
